...
| `.versailles/` workspace | The versioned tool-state directory (`config.json`, `contracts.json` with its top-level `predicates` map, `manifests.json`, `generated/`) loaded as one unit; no file is valid to interpret in isolation. | workspace-context |
