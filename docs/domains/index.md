...
```
 ...
 source code ───────▶ │      manifest-extraction │  per-language extractor
             ...
             │         (the .versailles/ workspace)   │  jointly-
             │                                        │  loaded file set
             └───▲──────────┬────────────┬────────↑───┘
                 │          │            │        │
                 │          │            │        │ contract
...
|---|---|---|---|---|---|
| 1 | [contract-language](contract-language.md) | The contract expression grammar, parser, AST, semantic validator, structured error contract, and the predicate-call cross-referencing check against the top-level `predicates` map in `contracts.json` (existence, arity, arg types) | workspace-context (full context for semantic checks), manifest-extraction (field resolution), predicate-registry (predicate data) | [specs/versailles.md](../specs/versailles.md) | [draft](../contracts/contract-language.contract.yaml) |
...
| 3 | [workspace-context](workspace-context.md) | The `.versailles/` workspace as a jointly-loaded unit: additive-only format policy (no version gates), `VersaillesContext` object, scoped extraction helper, staleness check orchestration | all three `.versailles/` data files (config.json, contracts.json, manifests.json) + generated/ | [specs/versailles.md](../specs/versailles.md) | [draft](../contracts/workspace-context.contract.yaml) |
...
| 5 | [predicate-registry](predicate-registry.md) | Declarative predicate declarations in `contracts.json`: validator-time `sourceRef` resolution (resolve-or-warn), name validity, no purity gate (ADR-0019); `sourceHash` dropped (ADR-0013) | workspace-context (joint load), contract-language (cross-referencing enforcement) | [specs/predicate-registry.md](../specs/predicate-registry.md) | [draft](../contracts/predicate-registry.contract.yaml) |
