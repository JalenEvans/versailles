...
- **Plannability gate** — a property-strategy clause (the strategy table above) is planned
  ...
  - *Sampling strategy* (supersedes the oracle-arity gate) — every guard oracle must
    ...
    route to one of **three joint-sampling strategies** (VERSAILLES-165):
    ...
    - **FIELD-BOUND** — a bothSideFieldRef equality `f == p` with at least one
  ...
  coupling that references a manifest-field operand, an unboundable coupling
