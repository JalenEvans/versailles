...
- Per-field access and readonly capture (ADR-0021): refreshed entries record `fieldAccess` (public/protected/private) and `fieldReadonly` (boolean) from TS source modifiers — the emission-soundness input model that lets the emitter decide plain `instance.<field>` access vs the deliberate `(instance as any).<field>` cast; legacy entries without these keys load permissively (default accessible/not readonly, ADR-0004/0018).
...
### Per-field access and readonly are captured from TS source modifiers

- **Given** a TypeScript component with mixed field modifiers (e.g. `private balance: number`, `public readonly sku: string`)
- **When** `versailles extract-manifests` runs
- **Then** the manifest entry records `fieldAccess: { balance: "private", sku: "public" }` and `fieldReadonly: { balance: false, sku: true }` from the source modifiers — so the emitter can decide plain `instance.<field>` access vs the deliberate `(instance as any).<field>` cast for non-public fields (build-spec §3.3, ADR-0021)

### Legacy entries without access/readonly load permissively

- **Given** a preserved manifest entry that predates the access/readonly keys (no `fieldAccess` / `fieldReadonly`)
- **When** the workspace is loaded and the emitter renders instance-field access
- **Then** the entry loads permissively and defaults to accessible/not-readonly — plain `instance.<field>` access, no cast — consistent with the permissive policy (ADR-0004, ADR-0018, ADR-0021)
...
| Date | Author | Change |
|------|--------|--------|
| 2026-08-11 | maintainer | Initial draft from build-spec §3.3, §7; ADR-0004/0005/0008/0009/0010 |
| 2026-08-13 | maintainer | Removed Linked Plans section — execution plans are tracked outside the public repo |
| 2026-08-17 | maintainer | Mirrored contract changes (PR fix/generator-emitter-runnability): method metadata recording, sourcePath persistence, sourceHash covers sorted field pairs + sorted method-signature records (method bodies still excluded) — backs VERSAILLES-20/21 |
| 2026-08-18 | maintainer | Pinned the canonical sourcePath semantic (VERSAILLES-24): covered entries store project-root-relative POSIX paths (src/order.ts), never source-root-relative and never absolute — so the generator's join(cwd, sourcePath) resolves to the real source file |
