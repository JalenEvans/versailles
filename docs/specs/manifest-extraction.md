...
- `must_not` record `sourcePath` relative to a source root (`order.ts` for `src/order.ts`) or as an absolute path — the canonical semantic is project-root-relative with POSIX separators (`src/order.ts`) (VERSAILLES-24).
